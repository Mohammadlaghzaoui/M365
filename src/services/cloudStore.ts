...
/** Push all syncable localStorage keys to the server. */
...
/** Debounced push — called after each local save. */
...
/** Quick connectivity check for the Settings UI. */
export async function testCloudSync(c: CloudSyncConfig): Promise<{ ok: boolean; detail: string }> {
  try {
    ...
    await res.json();
    return { ok: true, detail: 'Connected — your assessments will sync across browsers.' };
  } catch (e) {
