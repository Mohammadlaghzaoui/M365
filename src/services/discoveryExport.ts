  ...
  return [
    {
      ...
      rows: [
        ...
        ['Secure Score', result.secureScore ? `${result.secureScore.current}/${result.secureScore.max} (${result.secureScore.percent}%)` : 'n/a'],
        ['Cloud PCs (Windows 365)', result.cloudPCs ?? 0],
        ['Intune app protection policies', result.appProtection ?? 0],
        ['Named locations', result.namedLocations ?? 0],
        ['Sensitivity labels', result.sensitivityLabels ?? 0],
    ...
    {
      name: 'Admin Centers coverage',
      columns: ['Admin center', 'Collected via Microsoft Graph', 'Status', 'Notes / separate tool'],
      rows: [
        ['Identity (Entra ID)', `${result.users.length} users · ${result.groups.length} groups · ${(result.adminRoles ?? []).length} roles · ${result.caPolicies.length} CA · ${result.namedLocations ?? 0} named locations`, 'Collected', 'Full identity inventory.'],
        ['Security (Defender)', result.secureScore ? `Secure Score ${result.secureScore.percent}%` : 'CA policies only', result.secureScore ? 'Collected' : 'Partial', 'Alerts/incidents via Defender API if needed.'],
        ['Microsoft Purview', `${result.sensitivityLabels ?? 0} sensitivity labels`, (result.sensitivityLabels ?? 0) ? 'Partial' : 'Limited', 'DLP & retention need Purview/Security & Compliance PowerShell.'],
        ['Microsoft Intune', `${result.devices.total} devices · ${(result.compliancePolicies ?? []).length} compliance · ${result.intune?.configs ?? 0} configs · ${result.appProtection ?? 0} app protection · ${result.cloudPCs ?? 0} Cloud PCs`, 'Collected', 'Endpoint inventory + naming plan.'],
        ['Azure', 'VM/Cloud PC devices detected', 'Separate tool', 'Subscriptions, VMs & networks need Azure Resource Manager API.'],
        ['Exchange Online', `${result.usage.mailboxCount} mailboxes · ${result.usage.mailboxTotalGB.toFixed(0)} GB · aliases · shared mailboxes`, 'Partial', 'Permissions, rules, transport & connectors need Exchange Online PowerShell.'],
        ['SharePoint', `${result.sharePointSites.length} sites · ${result.usage.spoTotalGB.toFixed(0)} GB`, 'Collected', 'Sites + sharing exposure.'],
        ['Teams', `${result.teams} teams · channels · members`, 'Partial', 'Calling/meeting/messaging policies need Teams PowerShell.'],
        ['Power Platform', 'Environments, flows & apps', 'Separate tool', 'Needs the Power Platform Admin API.'],
      ],
    },
    {
      name: 'Endpoint Plan',
